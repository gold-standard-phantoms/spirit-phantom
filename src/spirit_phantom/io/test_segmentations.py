...
_VOLUME_SHAPE: Final = (100, 100, 100)
...
def _create_spherical_label_volume() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    and centre-to-centre spacing of 20 voxels. Each sphere is assigned a unique
    label value starting from 1.0.
    ...
    z_indices, y_indices, x_indices = np.indices(_VOLUME_SHAPE)
    ...
    for z_centre in _GRID_COORDINATES:
        for y_centre in _GRID_COORDINATES:
            for x_centre in _GRID_COORDINATES:
                ...
                centres_list.append((z_centre, y_centre, x_centre))
