...
def test_generate_dice_score_table_uses_manual_to_atlas_mapping(
    tmp_path: Path,
) -> None:
...
def test_generate_vial_segmentation_accuracy_table_partial_overlap(
    ...
    assert row_d["fnr"] == pytest.approx(0.5)
    assert row_d["fpr"] == pytest.approx(0.0)
...
def test_format_vial_segmentation_accuracy_table_renders_headers(
    ...
    assert table.index("dice_score") < table.index("fpr") < table.index("fnr")
    assert table.index("fnr") < table.index("manual_voxels")
