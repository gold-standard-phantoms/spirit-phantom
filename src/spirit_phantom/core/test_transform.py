...
try:
    ...
    itk = None
    transform_points_fixed_to_moving = None
else:
    from spirit_phantom.core.transform import transform_points_fixed_to_moving
...
def _create_moving_image(*, output_path: Path) -> None:
    ...
    """
    if itk is None:
        msg = "itk is required to create a moving image."
...
def _run_transform_pipeline(*, output_dir: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    Args:
        output_dir: Directory where all intermediate and output files are saved.

    Returns:
        Tuple of (actual_points, expected_points).
    """
    if transform_points_fixed_to_moving is None:
        msg = "itk is required to run the transform pipeline."
        raise RuntimeError(msg)
    ...
    save_points(points=fixed_points.tolist(), output_path=points_path, point_type="point")
...
@pytest.mark.skipif(itk is None, reason="itk is not installed in this environment")
...
def main() -> None:
    ...
    This is useful when manually validating transform behaviour and inspecting
    intermediate files generated by transformix.
    """
    if itk is None:
        msg = "itk is required to run main(). Install itk-elastix first."
        raise RuntimeError(msg)
