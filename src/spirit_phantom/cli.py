...
The registration command defaults to the bundled SPIRIT atlas image when a
moving image is not provided explicitly.
"""
...
DEFAULT_REGISTER_MOVING_IMAGE = (
    Path(__file__).resolve().parent
    / "core"
    / "configuration"
    / "spirit_issue1.0_vx0.25_sub2.84ada80.nii.gz"
)
...
def register(
    ...
    moving_image: Annotated[
        ...
        typer.Argument(
            help=(
                "Path to the moving (atlas) image. Defaults to the bundled "
                "SPIRIT atlas image."
            )
        ),
    ] = DEFAULT_REGISTER_MOVING_IMAGE,
    *,
    ...
    Args:
        ...
        moving_image: Path to the moving (atlas) image. Uses the bundled atlas
            by default.
